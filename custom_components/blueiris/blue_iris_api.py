...
class BlueIrisApi:
    ...
    def get_data(self):
        ...
        _LOGGER.debug(f'Status of BlueIris: {response}')
    ...
    def call_blue_iris_admin(self, request_data):
        ...
        try:
            ...
            _LOGGER.debug(f'Request to BlueIris sent to: {url}')
            r = requests.get(url, auth=self._auth, timeout=5)
            response = r.text
            ...
            _LOGGER.error(f'Failed to get response from BlueIris due to HTTP Error: {str(e)}')
        except Exception as ex:
            _LOGGER.error(f'Failed to get response from BlueIris due to unexpected error: {str(ex)}')
